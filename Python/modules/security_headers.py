...
def detect_http_auth_type(headers: Dict[str, str]) -> Optional[str]:
    """
    Detect HTTP authentication type from response headers
    ...
    # Check WWW-Authenticate header (case-insensitive)
    www_auth = None
    ...
    if 'ntlm' in www_auth_lower:
        return 'NTLM'
    elif 'negotiate' in www_auth_lower:
        return 'Negotiate (Kerberos/NTLM)'
    elif 'basic' in www_auth_lower:
        return 'HTTP Basic'
    elif 'digest' in www_auth_lower:
        return 'HTTP Digest'
    elif 'bearer' in www_auth_lower:
        return 'Bearer Token'
    else:
        return f'HTTP Auth ({www_auth[:50]})'
