...
class DB_Manager(object):
    ...
    def get_http_object_by_url(self, remote_system):
        """Check if a URL already exists in the database"""
        c = self.connection.cursor()
        for row in c.execute("SELECT * FROM http"):
            obj = pickle.loads(row['object'])
            if obj.remote_system == remote_system:
                c.close()
                return obj
        c.close()
        return None
    ...
    def create_http_object(self, remote_system, cli_parsed):
        ...
        # Check if URL already exists (for --db mode)
        existing_obj = self.get_http_object_by_url(remote_system)
        if existing_obj:
            # Update the existing object instead of creating new
            existing_obj.set_paths(cli_parsed.d, None)
            existing_obj.max_difference = cli_parsed.difference
            # Reset some fields for re-scan
            existing_obj._category = None
            existing_obj._default_creds = None
            existing_obj._error_state = None
            existing_obj._page_title = None
            existing_obj._ssl_error = False
            existing_obj.category = None
            existing_obj.default_creds = None
            existing_obj.error_state = None
            existing_obj.page_title = None
            existing_obj.source_code = None
            c.close()
            return existing_obj
    ...
    def recategorize(self, save_to_db=False):
        """
        Recategorize all HTTP objects in the database.
        
        Args:
            save_to_db (bool): If True, save updated objects back to database
        
        Returns:
            list: List of recategorized HTTPTableObject instances
        """
        finished = []
        counter = 0
        updated_count = 0
        ...
        for row in rows:
            ...
            for ua in uadat:
                ...
                if uao is not None:
                    o.add_ua_data(uao)
            # Recategorize all objects, not just those that aren't unauth/notfound
            # This ensures all categories are updated with new signatures
            old_category = o.category
            o = default_creds_category(o)
            new_category = o.category
            
            if old_category != new_category:
                print('{0} changed from {1} to {2}'.format(o.remote_system, old_category or 'None', new_category or 'None'))
                updated_count += 1
        ...
        if save_to_db and updated_count > 0:
            self.connection.commit()
            print(f'\n[*] Updated {updated_count} objects in database')
